...
class LoginTwoFactorTest extends WebTestCase
{
    ...
    public function testLogin2faRedirectWhenNoTotpEnrolled(): void
    {
		...
		$this->assertStringContainsString( '/admin', $current_location, 'User without 2FA should be redirected to /admin' );
    ...
    public function testLogin2faCheckWithValidCode(): void
    {
		...
		$this->assertStringNotContainsString( '2fa', $redirect, 'After successful 2FA, should not redirect back to /login/2fa' );
    }
    ...
    public function testLogin2faCheckWithInvalidCode(): void
    {
	    ...
	    $redirect = $this->client->getResponse()->headers->get( 'Location' );
	    $this->assertStringContainsString( '/login/2fa', $redirect, 'After invalid 2FA, should redirect back to /login/2fa' );

	...
	public function getCodecFields( $defaults = [] ): FieldCollection
	{
		return new FieldCollection( [
			'delimiter'       => [
				...
				'help'        => $this->trans( 'Sets the field delimiter separating values (one character only)' ),
				'type'        => 'text',
				'placeholder' => ',',
				'default'     => $defaults['delimiter'] ?? null,
			],
			'enclosure'       => [
				'label'       => $this->trans( 'Enclosure' ),
				'help'        => $this->trans( 'Sets the field enclosure (one character only)' ),
				'type'        => 'text',
				'placeholder' => '"',
				'default'     => $defaults['enclosure'] ?? null,
			],
			'escape_char'     => [
				'label'      => $this->trans( 'Escape character' ),
				'help'       => $this->trans( 'Sets the escape character (at most one character)' ),
				'type'       => 'text',
				'default'    => $defaults['escape_char'] ?? null,
			],
			'end_of_line'     => [
				'label'       => $this->trans( 'End of line' ),
				'help'        => $this->trans( 'Sets the character(s) used to mark the end of each line in the CSV file' ),
				'type'        => 'text',
				'placeholder' => '\\n',
			...
			'key_separator'   => [
				'label'       => $this->trans( 'Key separator' ),
				'help'        => $this->trans( 'Sets the separator for array\'s keys during its flattening' ),
				'type'        => 'text',
				'placeholder' => '.',
				'default'     => $defaults['key_separator'] ?? null,
			],
			'escape_formula'  => [
				'label'      => $this->trans( 'Escape formula' ),
				'help'       => $this->trans( 'Escapes fields containing formulas by prepending them with a `\t` character' ),
				'type'       => 'checkbox',
				'default'    => $defaults['escape_formula'] ?? null,
			],
			'no_headers'      => [
				'label'      => $this->trans( 'No headers' ),
				'help'       => $this->trans( 'Disables header in the encoded CSV' ),
				'type'       => 'checkbox',
				'default'    => $defaults['no_headers'] ?? null,
			],
			'as_collection'   => [
				'label'      => $this->trans( 'As collection' ),
				'help'       => $this->trans( 'Always returns results as a collection, even if only one line is decoded.' ),
				'type'       => 'checkbox',
				'default'    => $defaults['as_collection'] ?? true,
				'_direction' => 'decode',
			],
			'output_utf8_bom' => [
				'label'      => $this->trans( 'Output UTF8 Bom key' ),
				'type'       => 'checkbox',

...
class OAuth2 extends WebserviceModel
{
	...
	private function extractTokens( ResponseInterface $response ): Tokens
	{
		...
		try {
			...
			throw new \Exception( 'Error parsing token endpoint JSON response. JSON error is: '
			                      . $e->getMessage(), 0, $e, );
		}
		...
		return [
			...
			&& is_string( $token['refresh_token'] ) ? $token['refresh_token'] : null,
		];
	}
